...
import { ActivityStatsCards } from '@/features/activities/components/ActivityStatsCards';
...
import { ActivityTypeMenu } from '@/features/activities/components/ActivityTypeMenu';
import { CreateCallDialog, type CallData } from '@/features/activities/components/CreateCallDialog';
import { SendEmailDialog, type EmailData } from '@/features/activities/components/SendEmailDialog';
import { SendTelegramDialog, type TelegramData } from '@/features/activities/components/SendTelegramDialog';
...
export const ActivitiesPage = () => {
  ...
  const loadVideoCalls = async () => {
    try {
      ...
      console.log('[ActivitiesPage] Video calls array check:', Array.isArray(calls));
      setVideoCalls(calls || []);
    } catch (error) {
      ...
      toaster.create({
        title: 'Warning',
        description: 'Could not load call history. Activities will be shown without calls.',
        type: 'warning',
        duration: 3000,
      });
    }
