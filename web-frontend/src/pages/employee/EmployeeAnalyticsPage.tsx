...
const EmployeeAnalyticsPage = () => {
  ...
  const handleExport = async () => {
    try {
      // Get analytics data
      const [dashboardStats, revenueData, topPerformers] = await Promise.all([
      ...
      const exportRows = [
        // Dashboard Stats
        { Metric: 'Total Customers', Value: dashboardStats.customers || 0 },
        { Metric: 'Total Leads', Value: dashboardStats.leads || 0 },
        { Metric: 'Total Deals', Value: dashboardStats.deals || 0 },
        // Revenue Data
        ...revenueData.monthlyData.map((month: any) => ({
          Metric: `Revenue - ${month.name}`,
